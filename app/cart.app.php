...
class CartApp extends MallbaseApp {
    ...
    function add() {
        ...
        $spec_info['price'] = $this->get_spec_price($spec_info);
    ...
    function meals_drop($spec_id, $quantity, $store_id) {
        ...
        /* 从购物车中删除 */
        $model_cart = & m('cart');
        $droped_rows = $model_cart->drop('spec_id=' . $spec_id . ' AND session_id=\'' . SESS_ID . '\'', 'store_id');
        if (!$droped_rows) {
            return;
        }
    }
    ...
    function meals_clear()
    {
        $model_cart = & m('cart');
        $store_id = isset($_GET['store_id']) ? intval($_GET['store_id']) : 0;
        $droped_rows = $model_cart->drop('store_id=' . $store_id . ' AND session_id=\'' . SESS_ID . '\'', 'store_id');
        if (!$droped_rows) {
            return;
        }else{
            $cartShowDiv = '<div class="i i-empty">购物车为空</div>';
    ...
    function max_goods_cart() {
        ...
        foreach ($spec_ids as $key => $spec_id) {
            ...
            //当用多个促销条件存在时候， 此方法用来处理具体使用哪一个价格
            $total_price += $this->get_spec_price($spec_info);
            $total_count++;
        }
        $this->json_result(array(
