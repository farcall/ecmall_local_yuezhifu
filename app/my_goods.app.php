...
//            )));
    ...
    /**
     * 当产品添加为促销产品后,如重新编辑编辑产品规格,促销会出现BUG
     * @param type $id
     */
    function _check_edit_power($goods_id) {
        $promotion = $this->_promotion_mod->get('goods_id='.$goods_id);
        if($promotion){
            $this->show_warning(sprintf(Lang::get('edit_promotion_limit'), $promotion['pro_name']));
            exit;
        }
        ...
        if (!IS_POST) {
            ...
            foreach ($records as $record) {
                ...
                if ($record['sale_attr']) { // 有规格
                    ...
                    if ($spec_qty > 2 || !$spec_info) { // 有两个以上规格或淘宝接口没有获取到属性，视无规格处理
                        $spec_qty = 0;
                        $spec_data = array();
                        $spec_data[0] = array(
                            'goods_id' => $goods_id,
    ...
    function _parse_taobao_csv($csv_string) {
        ...
        foreach ($records as $key => $record) {
            foreach ($record as $k => $col) {
                ...
                /* 对字段数据进行分别处理 */
                switch ($k) {
                    case $fields_cols['description'] : $return[$key]['description'] = str_replace(array("\\\"\\\"", "\"\""), array("\\\"", "\""), $col);
                        break;
                    case $fields_cols['goods_image'] : $result = $this->_parse_taobao_image($col);
                        $return[$key]['goods_image'] = $result['data'];
                        $return[$key]['image_count'] = $result['count'];
                        break;
                    case $fields_cols['if_show'] : $return[$key]['if_show'] = $col == 1 ? 0 : 1;
                        break;
                    case $fields_cols['goods_name'] : $return[$key]['goods_name'] = $col;
                        break;
                    case $fields_cols['stock'] : $return[$key]['stock'] = $col;
                        break;
                    case $fields_cols['price']: $return[$key]['price'] = $col;
                        break;
                    case $fields_cols['recommended'] : $return[$key]['recommended'] = $col;
                        break;
                    case $fields_cols['sale_attr'] : $return[$key]['sale_attr'] = $col;
                        break;
                    case $fields_cols['sale_attr_alias'] : $return[$key]['sale_attr_alias'] = $col;
                        break;
                    case $fields_cols['cid'] : $return[$key]['cid'] = $col;
                        break;
                }
            }
        }
...
//            ),
    ...
    function _get_post_data($id = 0) {
        ...
        if($market_price<=0){
            if(is_array($_POST['price'])){
                $market_price = $this->_filter_price(current($_POST['price'])*1.2);
            }else{
                $market_price = $this->_filter_price($_POST['price']*1.2);
            }
        }
        
        $goods = array(
            'goods_name' => $_POST['goods_name'],
            'goods_subname' => $_POST['goods_subname'],
    ...
    function brand_list() {
        ...
        $this->_format_page($page);
        $this->_curlocal(LANG::get('member_center'), 'index.php?app=member', LANG::get('my_goods'), 'index.php?app=my_goods', LANG::get('brand_list'));
        $this->_curitem('my_goods');
    ...
    function recommend() {
        if (!IS_POST) {
            $this->_curlocal(LANG::get('member_center'), 'index.php?app=member', LANG::get('my_goods'), 'index.php?app=my_goods', LANG::get('recommend'));
            $this->_curitem('my_goods');
            ...
            $ret_page = isset($_GET['ret_page']) ? intval($_GET['ret_page']) : 1;
            $this->show_message('recommend_ok', 'back_list', 'index.php?app=my_goods&page=' . $ret_page, 'view_recommended_goods', 'index.php?app=my_recommend&amp;act=view_goods&amp;id=' . $recom_id);
        }
    }
