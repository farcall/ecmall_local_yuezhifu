...
class GcategoryApp extends BackendApp
{
    ...
    function add()
    {
        ...
        else
        {
            ...
            //处理上传的分类LOGO
            $cate_logo       =   $this->_upload_logo($cate_id);
            if ($cate_logo === false)
            {
                return;
            }
            $cate_logo && $this->_gcategory_mod->edit($cate_id, array('cate_logo' => $cate_logo)); //将logo地址记下
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            if($gcategory['cate_logo']){
                $gcategory['cate_logo']  =   dirname(site_url()) . "/" . $gcategory['cate_logo'];
            }
        ...
        else
        {
            ...
            $cate_logo   =   $this->_upload_logo($id);
            $cate_logo && $data['cate_logo'] = $cate_logo;
            if ($cate_logo === false)
            {
                return;
            }
    ...
    function _upload_logo($cate_id)
    {
        $file = $_FILES['cate_logo'];
        ...
        $uploader->addFile($_FILES['cate_logo']);//上传logo
        if (!$uploader->file_info())
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=gcategory&amp;act=edit&amp;id=' . $cate_id);
            return false;
        }
        /* 指定保存位置的根目录 */
        $uploader->root_dir(ROOT_PATH);
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/gcategory', $cate_id))  
        {
            return $file_path;
        }
        else
